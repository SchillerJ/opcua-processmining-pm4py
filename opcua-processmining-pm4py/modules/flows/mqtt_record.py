...
class MqttFixedHeader():
    ...
    def __get_client_ids(self, packet) -> dict:
        ...
        for key, value in client_keys.items():
            ...
            client_ids[key] = client_id if client_id is not None else "Unknown client"
    ...
    def __parse_correlation_data(self, tcp_payload: str, control_type: int, qos: int) -> str or None:
        if control_type == 3:
            ...
            try:
                ...
                return correlation_data.decode('utf-8') if correlation_data is not None else None
        return None
...
class MqttRecord(FlowRecord):
    ...
    def __init__(self, packet):
        ...
        super(MqttRecord,self).__init__(packet)
    ...
    def __get_record_id(self):
        ...
        FlowTuple = namedtuple("FlowTuple", ["src_ip", "src_port", "dst_ip",
                                             "dst_port", "protocol", "packet_id"])
        
        # packet identifier only available if qos > 0
        packet_id = self.variable_header.packet_id if self.variable_header.packet_id is not None else "n/a"

        # normal control flow
        if self.fixed_header.control_type not in reversed_flows:
            quintuple = FlowTuple(src_ip=self.layer3_and_4.source_ip,
                                  src_port=self.layer3_and_4.source_port,
                                  dst_ip=self.layer3_and_4.destination_ip,
                                  dst_port=self.layer3_and_4.destination_port,
                                  protocol=self.layer3_and_4.protocol,
                                  packet_id=packet_id)

        # reversed control flow
        else:
            quintuple = FlowTuple(src_ip=self.layer3_and_4.destination_ip,
                                  src_port=self.layer3_and_4.destination_port,
                                  dst_ip=self.layer3_and_4.source_ip,
                                  dst_port=self.layer3_and_4.source_port,
                                  protocol=self.layer3_and_4.protocol,
                                  packet_id=packet_id)
