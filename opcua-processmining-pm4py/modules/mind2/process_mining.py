...
def start_process_mining():
    ...
    rows = []
    for flow in list(_db.collection.find({"mqttControlType": 3}).limit(limit)):
    ...
    _df = pd.DataFrame(rows, columns=['time:timestamp', 'case:concept:name', 'case:clientID', 'concept:name'])
    event_log = pm4py.convert_to_event_log(_df)
    print(_df)

    _net, _im, _fm = petri_net_heuristic_miner(event_log, 0.7)
    ...
    #    parameters={"format": "svg"})
    # pm4py.visualization.petri_net.visualizer.view(gviz)
    # directly_follows_graph(event_log)
    petri_net_inductive_miner(event_log, 0.99)
